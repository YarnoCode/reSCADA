    ...
    //:Option(Option)
{
    ...
    if( Option & PIDopt::kP ) kP = new OutETag( Owner, Prom::TpOut, Prom::PreSet, Name + " коэф. П",  TagPrefix + TagsNames->at(pid::kP),false,false,false,true,Prom::VCNo,false,false,0,true);
    if( Option & PIDopt::kI ) kI = new OutETag( Owner, Prom::TpOut, Prom::PreSet, Name + " коэф. И",  TagPrefix + TagsNames->at(pid::kI),false,false,false,true,Prom::VCdiv1000,false,false,0,true);
    if( Option & PIDopt::kD ) kD = new OutETag( Owner, Prom::TpOut, Prom::PreSet, Name + " коэф. Д",  TagPrefix + TagsNames->at(pid::kD),false,false,false,true,Prom::VCdiv1000,false,false,0,true);
    ...
    if( Option & PIDopt::manImp )manImp = new OutETag( Owner, Prom::TpOut, Prom::PreSet, ImpName + " руч. упр-е",  TagPrefix + TagsNames->at(pid::manImp),false,false,false,true,Prom::VCNo,false,false,0,true);
    ...
    if( Option & PIDopt::impLimMax )impMax = new OutETag( Owner, Prom::TpOut, Prom::PreSet, ImpName + " макс. воздей-е",  TagPrefix + TagsNames->at(pid::impMax),false,false,false,true,Prom::VCNo,false,false,0,true);
    if( Option & PIDopt::impLimMin )impMin = new OutETag( Owner, Prom::TpOut, Prom::PreSet, ImpName + " мин. воздей-е",  TagPrefix + TagsNames->at(pid::impMin),false,false,false,true,Prom::VCNo,false,false,0,true);
    if( Option & PIDopt::feedback )feedback = new InETag( Owner, Prom::TpIn, Name + " ответ от управляемого устройства",  TagPrefix + TagsNames->at(pid::feedback), true, 100, 0.5, false, false, false, false, true);
}
//------------------------------------------------------------------------------
